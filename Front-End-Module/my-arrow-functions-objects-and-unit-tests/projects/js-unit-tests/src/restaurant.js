...
const createMenu = (menu) => {
  const myRestaurant = {
    ...
    pay: () => {
      let sumOfOrdersConsumption = 0;
      myRestaurant.consumption.forEach((item) => {
        for (let count = 0; count < Object.keys(myRestaurant.fetchMenu()).length; count += 1) {
          if (Object.keys(myRestaurant.fetchMenu().food)[count] === item) {
            sumOfOrdersConsumption += Object.values(myRestaurant.fetchMenu().food)[count];
          }
          if (Object.keys(myRestaurant.fetchMenu().drink)[count] === item) {
            sumOfOrdersConsumption += Object.values(myRestaurant.fetchMenu().drink)[count];
          }
        }
      });
      const taxService = Number((sumOfOrdersConsumption * 1.1).toFixed(2));
      return taxService;
    },
  };
  return myRestaurant;
